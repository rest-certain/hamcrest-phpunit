...
final class MapMatchesKey extends Constraint
{
    private Constraint $keyConstraint;

    public function __construct(mixed $keyConstraint)
    {
    ...
    protected function failureDescription(mixed $other): string
    {
        ...
        return sprintf('value is a hash map with a key that %s', $this->keyConstraint->toString());
    }
    ...
    protected function matches(mixed $other): bool
    {
        ...
        foreach ($other as $key => $value) {
            if ($this->keyConstraint->evaluate($key, '', true)) {
                return true;
            }
        }
