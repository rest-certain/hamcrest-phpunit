...
final class MapMatchesKeyValuePair extends Constraint
{
    private Constraint $keyConstraint;
    private Constraint $valueConstraint;

    public function __construct(mixed $keyConstraint, mixed $valueConstraint)
    {
        if (!$keyConstraint instanceof Constraint) {
            $keyConstraint = new IsEqual($keyConstraint);
        }
    ...
    protected function failureDescription(mixed $other): string
    {
        ...
        return sprintf(
            'value is a hash map with a key that %s having a value that %s',
    ...
    protected function matches(mixed $other): bool
    {
        ...
        foreach ($other as $key => $value) {
            if ($this->keyConstraint->evaluate($key, '', true) && $this->valueConstraint->evaluate($value, '', true)) {
                return true;
            }
        }
